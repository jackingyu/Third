...
public class DefaultReservationDao extends GenericDAO<ReservationModel, String>
	...
	@Override
	public PaginationSupport findReservations(final String storeCode,
			final String cellphone, final String name, final Date fromDate,
			final Date toDate, Integer startIndex, Integer pageSize)
	{
		DetachedCriteria dc = DetachedCriteria.forClass(ReservationModel.class);

		if (!StringUtils.isEmpty(cellphone))
			dc.add(Restrictions.like("cellphone",
	...
	{
		Long numberOfReservation = (Long) this.currentSession()
				.createQuery(COUNT_CUST_RESERVATION)
