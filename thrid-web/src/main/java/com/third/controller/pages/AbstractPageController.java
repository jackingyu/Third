...
public abstract class AbstractPageController {
	...
	protected void fillAllStoreInView(final Model model,
			final List<StoreData> selectedStores)
	{
		List<ComboboxData> stores = new ArrayList<ComboboxData>();
	...
	/**
	 * fill authorized store in view,set selected according the parameter
	 * storeCode
	 * 
	 * @param model
	 * @param storeCode
	...
	{
		List<ComboboxData> productGroups = convertProductGrouptoCombobox(
				productFacade.getProductGroups());
	...
	protected void fillSourceInModel(final Model model)
	{
		...
		List<SourceData> sourceDatas = sourceFacade
				.getSourceForStoreCode(user.getStores().get(0).getCode());
	...
	protected List<ComboboxData> convertProductGrouptoCombobox(
