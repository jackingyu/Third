...
public class DefaultProductFacade implements ProductFacade {
	...
	@Override
	public DTResults getProductList(Map<String, String> sp,
			final Integer startIndex, final Integer pageSize)
	{
		PaginationSupport ps = productService.getProductList(sp, startIndex,
				pageSize);
	...
	@Override
	public DTResults getProductList1(Map<String, String[]> sp,
			final Integer startIndex, final Integer pageSize)
	{
		PaginationSupport ps = productService.getProductList1(sp, startIndex,
				pageSize);
	...
	{
		List<ProductGroupModel> productGroupModels = productGroupService
				.getProductGroupList();
	...
	public void setProductGroupDataPopulator(
			ProductGroupDataPopulator productGroupDataPopulator)
	{
	...
	public void setCategoryDataPopulator(
			CategoryDataPopulator categoryDataPopulator)
	{
	...
	public void saveProduct(ProductData productData)
	{
		...
		if (StringUtils.isNotEmpty(productData.getCode()))
			productModel = productService
					.getProductForCode(productData.getCode());

		if (productModel == null)
			productModel = new ProductModel();

		ProductGroupModel productGroupModel = productGroupService
				.getProductGroupByPK(productData.getProductGroup().getPk());
		CategoryModel categoryModel = categoryService
				.getCategoryForCode(productData.getCategory().getCode());
